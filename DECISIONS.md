...
- Realtime "today board": implemented as 30-second client-side polling (`router.refresh()` on an interval) rather than a Supabase Realtime channel — `postgres_changes` subscriptions are themselves RLS-scoped, so an employee's Team-today subscription couldn't see coworkers' row changes anyway (their RLS policy only allows reading their own `time_entries`/`breaks` rows); the owner dashboard would work over Realtime, but 30s polling covers both screens with one simple mechanism, which the spec explicitly allows as a fallback.
